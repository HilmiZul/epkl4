      ...
      <div class="row">
        <div class="col-md-6">
          ...
          <form @submit.prevent="buatPembimbingBaru">
            ...
            <button :disabled="isSending || form.username.length < 3 || form.email.length < 10 || form.password.length < 8 || form.nama.length < 4 || form.role.length < 4" class="btn btn-dark me-2 border border-2 border-dark me-3">
        ...
        <div class="col-md-6">
          <div class="alert alert-secondary border-5 border-dark">
            <ul class="mb-0">
              <li>Guru pembimbing adalah Guru yang mengajar tingkat XII</li>
              <li>Guru mapel Umum, <strong>Program Keahlian</strong> bisa lebih dari satu</li>
              <li>Guru pembimbing termasuk Guru kejuruan</li>
              <li>Guru pembimbing dipetakan sebagai PIC ke IDUKA</li>
              <li>Guru Pembimbing dipetakan ke peserta sesuai konversi JJM</li>
            </ul>
          </div>
        </div>
      </div>
...
if(user?.user.value.role != 'admin' && user?.user.value.role != 'wakasek') navigateTo('/404')
...
async function getProkelByIdAndStudentByProkel(loading=true) {
  ...
  if(data) {
    ...
    // mengambil jumlah siswa untuk perhitungan konversi jjm ke jumlah siswa yang dibimbing
    let res_students = await client.collection('siswa').getList(1, 1)

    if(res_students) {
      // variable ini dipake buat hitung konversi JJM ke Jumlah Siswa
